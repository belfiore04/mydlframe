...
class CSManager:
    ...
    def forward(self, pkt: Pkt):
        try:
            # 打印转发信息
            print(
                f"[CSManager] Forwarding packet from {pkt.from_addr} to {pkt.to_addr} with data: {pkt.data.decode('utf-8')}")
            # 根据实际实现调用发送方法
            if self.server.has_link(pkt.to_addr):
                self.server.send(pkt)
            elif pkt.to_addr in self.clients:
                self.clients[pkt.to_addr].send(pkt)
            else:
                print(f"[CSManager] No link found for {pkt.to_addr}")
        except Exception as e:
            print(f"[CSManager] Exception in forward: {e}")